...
def plot_maze(maze='nav01', margin=1, means=None, stds=None, figure_name=None, show=False, pause=False, ax=None, linewidth=1.0):
    ...
    if 'gdc01' in maze:
        walls = np.loadtxt("/content/drive/MyDrive/differentiable-particle-filter/GDC1.txt", dtype='float', delimiter=',')
        walls =np.reshape(walls, (-1, 2, 2))
        rooms = [
            # [[400, 200], 300, 200]
            ]
        # ax.set_xlim([-margin, 100+margin])
        ax.set_ylim([-margin, 25+margin])
    if 'nav01' in maze:
        walls = np.array([
            # horizontal
            [[0, 500], [1000, 500]],
